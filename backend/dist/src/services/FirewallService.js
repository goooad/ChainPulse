...
class FirewallService {
    constructor() {
        ...
        try {
            ...
            this.provider = new ethers_1.ethers.JsonRpcProvider('https://rpc.ankr.com/eth');
        }
    }
    async startMonitoring() {
        ...
        try {
            ...
            console.warn('⚠️  实时监控启动失败，使用模拟数据:', error.message);
            this.startMockMonitoring();
        }
    }
    startMockMonitoring() {
    ...
    async checkContractRisk(address) {
        const reasons = [];
        let score = 0;
        try {
            // 检查是否为已知恶意合约
    ...
    checkValueRisk(value) {
        const reasons = [];
        let score = 0;
        const ethValue = Number(ethers_1.ethers.formatEther(value));
        // 检查异常大额交易
        if (ethValue > 100) {
            score += 30;
    ...
    async checkAddressRisk(address) {
        const reasons = [];
        let score = 0;
        try {
            // 检查地址是否在黑名单中
